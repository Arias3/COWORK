...
class RobleApiDataSource {
  ...
  try {
    ...
    final headers = isAuthRequest ? RobleConfig.authHeaders : RobleConfig.dataHeaders;

    if (method.toUpperCase() == 'PUT') {
      print('=== DEBUG PUT REQUEST ===');
      print('URL: $baseUrl/$endpoint');
      print('Headers enviados: $headers');
      print('AccessToken en RobleConfig: ${RobleConfig.accessToken}');
      print('Body: $body');
      print('========================');
    }
    ...
    if (response.statusCode >= 200 && response.statusCode < 300) {
      ...
      return jsonDecode(response.body); // Retorna dynamic (puede ser Map o List)
    } else {
  ...
  final response = await _makeRequest('POST', 'insert', body: {
    'tableName': tableName,
    'records': [data],
  ...
  Future<List<Map<String, dynamic>>> read(String tableName, {Map<String, dynamic>? filters}) async {
    ...
    final response = await _makeRequest('GET', 'read', queryParams: queryParams);
  ...
  Future<Map<String, dynamic>> update(String tableName, dynamic id, Map<String, dynamic> data) async {
  ...
  return await _makeRequest('PUT', 'update', body: {
    'tableName': tableName,
 ...
 Future<Map<String, dynamic>> delete(String tableName, dynamic id) async {
  return await _makeRequest('DELETE', 'delete', body: {
    'tableName': tableName,
