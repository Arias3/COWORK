...
class _EvaluacionDetallePageState extends State<EvaluacionDetallePage>
  ...
  Future<void> _cargarDatos() async {
    ...
    try {
      ...
      await _evaluacionController.cargarEvaluacionPeriodo(widget.evaluacion.id);

      // Obtener equipos asignados a esta actividad
      final asignaciones = await _equipoActividadUseCase
          .getAsignacionesByActividad(widget.activity.id!);
  ...
  Widget _buildResumenTab() {
    return Obx(() {
      final estadisticas = _evaluacionController.estadisticas;
            ...
            Container(
              ...
              child: Row(
                children: [
                  ...
                  SizedBox(width: 8),
                  Text(
                    'Tiempo restante: ${evaluacion.horasRestantes} horas',
                    style: TextStyle(
  ...
  Widget _buildPorEquipoTab() {
    return Obx(() {
      final promediosEquipos = _evaluacionController.promediosEquipos;
              ...
              children: estudiantes.map((estudiante) {
                final promedioEstudiante =
                    _evaluacionController
                        .promediosEstudiantes[estudiante['id']] ??
                    0.0;
  ...
  Widget _buildPorEstudianteTab() {
    return Obx(() {
      final promediosEstudiantes = _evaluacionController.promediosEstudiantes;
  ...
  Future<void> _iniciarEvaluacion() async {
    await _evaluacionController.iniciarEvaluacion(widget.evaluacion.id);
    await _cargarDatos();
  }
